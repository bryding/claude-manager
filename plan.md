...
- [ ] Delete the `existingPlanSection` GroupBox computed property
- [ ] Create a new `ExistingPlanBanner` view component (inline in SetupView or separate file)
- [ ] Design the banner as a horizontal HStack with: icon, text showing task count, "Use Existing Plan" button, and "Dismiss" button
- [ ] Style the banner with a subtle background color (e.g., `.secondary.opacity(0.1)`) and rounded corners
- [ ] Position the banner between the header and project selection sections
- [ ] Add dismiss functionality that sets `context.existingPlan = nil`
- [ ] Ensure the banner doesn't disrupt the form flow or take excessive vertical space
